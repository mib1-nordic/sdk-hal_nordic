...
/**
 * @defgroup nrfx_gppi_fork_example Fork GPPI example
 * @{
 * @ingroup nrfx_gppi_examples
 *
 * @brief Example showing basic fork functionality of a nrfx_gppi helper.
 *
...
int main(void)
{
    ...
    /*
     * Initialize the output pin associated with the primary task.
     * The SET task will turn the LED on, CLR will turn it off, and OUT will toggle it.
    */
     ...
     * The SET task will turn the LED off, CLR will turn it on, and OUT will toggle it.
     */
    ...
    /* Create variable desired_ticks to store the output of nrfx_timer_ms_to_ticks function. */
    uint32_t desired_ticks = nrfx_timer_ms_to_ticks(&timer_inst, TIME_TO_WAIT_MS);
    NRFX_LOG_INFO("Time to wait: %lu ms", TIME_TO_WAIT_MS);

    /*
     * Set the timer channel NRF_TIMER_CC_CHANNEL0 in the extended compare mode to clear
    ...
    */
    status = nrfx_gppi_conn_alloc(
        ...
        nrfx_gpiote_out_task_address_get(&gpiote_inst, GPPI_OUTPUT_PIN_PRIMARY), &gppi_h);
    NRFX_ASSERT(status == 0);
    ...
    */
    nrfx_gppi_ep_attach(nrfx_gpiote_out_task_address_get(&gpiote_inst, GPPI_OUTPUT_PIN_FORK), gppi_h);
