...
static nrfx_twis_t twis_inst = NRFX_TWIS_INSTANCE(NRF_TWIS_INST_GET(TWIS_INST_IDX));
...
NRFX_INSTANCE_IRQ_HANDLER_DEFINE(twis, TWIS_INST_IDX, &twis_inst);

/* Define an IRQ handler named nrfx_twim_<TWIM_INST_IDX>_irq_handler. */
NRFX_INSTANCE_IRQ_HANDLER_DEFINE(twim, TWIM_INST_IDX, &twim_inst);
#endif
...
int main(void)
{
    ...
    nrfx_twim_config_t twim_config = NRFX_TWIM_DEFAULT_CONFIG(TWIM_TWIS_MASTER_SCL_PIN,
                                                              TWIM_TWIS_MASTER_SDA_PIN);
